...
variable "vm_size" {
  description = "FortiAnalyzer VM Size"
  type        = string
  ...
  validation {
    ...
    error_message = "Invalid VM size. Please refer to Fortinet Azure fortianalyzer Instance type support Document."
  }
}
...
variable "image" {
  ...
  - `version`                 - (`string`, optional, defaults to `7.2.9`) Fortinet FortiOS version; list available with 
                                `az vm image list --publisher Fortinet --offer fortinet-fortianalyzer --all --out table` command.
  - `publisher`               - (`string`, optional, defaults to `fortinet`) the Azure Publisher identifier for an image
                                which should be deployed.
  - `offer`                   - (`string`, optional, defaults to `fortinet-fortianalyzer`) the Azure Offer identifier corresponding to a
                                published image.
  - `sku`                     - (`string`, optional, defaults to `fortinet-fortianalyzer`) Fortinet fortianalyzer SKU; list available with
                                `az vm image list --publisher Fortinet --offer fortinet-fortianalyzer --all --out table` command.
