resource "azurerm_linux_virtual_machine" "main" {
  ...
  admin_username                  = var.secrets_already_exist == false ? azurerm_key_vault_secret.username[0].value : var.vm_username
  admin_password                  = var.secrets_already_exist == false ? azurerm_key_vault_secret.password[0].value : var.vm_password
  allow_extension_operations      = var.allow_extension_operations #FortiAuthenticator Custom Marketplace Image does not support extensions therefore disabling them.
