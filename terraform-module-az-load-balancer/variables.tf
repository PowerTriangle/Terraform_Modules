variable "frontend_ips" {
  ...
  **Inbound rules.**
  ...
  - `name` : Optional name to give to the rule.
  ...
  - `public_ip_name` : optional if `create_public_ip` is `true`, required if not. If creating a public IP, its possible to specify a custom IP name using this field, otherwise a default value will be used. If using an existing IP, this needs to be set to the name of the public IP to use.
...
variable "probes" {
  description = "Health probes you want to be created on the load balancer, the key is the name of the health probe."
  type = map(object({
...
variable "probe_protocol" {
  description = "Protocol of the load balancer probe."
  default     = "Tcp"
  type        = string
}

variable "probe_interval_in_seconds" {
  description = "Interval of the load balancer probe."
  default     = 5
  type        = number
}
...
variable "frontend_ips_order" {
  description = "When importing existing load balancers into the state, it is required to preserve the order of the frontend IP configurations in the load balancer. The maps use lexical sorting, which is a problem in some cases. Using this list allows the order to be preserved during the import stage."
  type        = list(string)
  default     = null
}
